...
export default function Cotizaciones({ triggerCreate, selectedBodega = 'Todas las bodegas' }: CotizacionesProps) {
  ...
  const generarNumeroCotizacion = () => {
    const siguienteNumero = cotizaciones.length + 1;
    return `COT-${String(siguienteNumero).padStart(3, '0')}`;
  };
  ...
  // Filtrar cotizaciones por bodega Y por búsqueda
  const filteredCotizaciones = useMemo(() => {
    return cotizaciones
      .filter((cotizacion) => selectedBodega === 'Todas las bodegas' || cotizacion.bodega === selectedBodega)
      .filter((cotizacion) =>
  ...
  // Crear nuevo cliente desde el modal de cotización
  ...
  const handleAgregarProducto = () => {
    ...
    // El precio ahora es opcional, puede ser 0
    if (precioProducto < 0) {
  ...
  const calcularTotales = useMemo(() => {
    ...
    productosOrden.forEach(item => {
      ...
      // Calcular el subtotal sin IVA para este producto
      const subtotalProducto = item.subtotal / (1 + ivaProducto / 100);
      // Calcular el IVA de este producto
    ...
    return { subtotal: subtotalSinIva, impuestos: totalImpuestos, impuestosPorPorcentaje, total, items };
  }, [productosOrden]);

  const handleCreate = () => {
    if (!formData.numeroCotizacion || !formData.cliente || !formData.fecha || !formData.fechaVencimiento || !formData.bodega) {
      toast.error('Por favor completa todos los campos obligatorios');
      return;
    }
  ...
  const handleEdit = () => {
    if (!selectedCotizacion || !formData.numeroCotizacion || !formData.cliente || !formData.bodega) {
      toast.error('Por favor completa todos los campos obligatorios');
      return;
    }
    ...
    setCotizaciones(cotizaciones.map(cotizacion =>
      cotizacion.id === selectedCotizacion.id
        ? { 
            ...cotizacion, 
            ...formData, 
            items: calcularTotales.items,
  ...
  const handleDelete = () => {
    ...
    setCotizaciones(cotizaciones.filter(cotizacion => cotizacion.id !== selectedCotizacion.id));
    toast.success('Cotización eliminada exitosamente');
    setIsDeleteModalOpen(false);
    setSelectedCotizacion(null);
  };

  const handleDownloadPDF = (cotizacion: Cotizacion, incluirPrecios: boolean = true) => {
    const doc = new jsPDF();
    ...
    // Columna derecha
    doc.text(`Fecha: ${new Date(cotizacion.fecha).toLocaleDateString('es-CO')}`, 120, 40);
    doc.text(`Fecha Vencimiento: ${new Date(cotizacion.fechaVencimiento).toLocaleDateString('es-CO')}`, 120, 46);
    ...
    if (incluirPrecios) {
      ...
      doc.text(`$${cotizacion.subtotal.toLocaleString('es-CO', { minimumFractionDigits: 2 })}`, 190, totalesY, { align: 'right' });
      
      // Desglose de IVA por porcentaje
      const impuestosPorPorcentaje = calcularImpuestosPorPorcentaje(cotizacion);
      const porcentajesOrdenados = Object.entries(impuestosPorPorcentaje).sort(([a], [b]) => Number(a) - Number(b));
      
      totalesY += 6;
      porcentajesOrdenados.forEach(([porcentaje, monto]) => {
        doc.text(`Total IVA ${porcentaje}%:`, 130, totalesY);
        doc.text(`$${monto.toLocaleString('es-CO', { minimumFractionDigits: 2 })}`, 190, totalesY, { align: 'right' });
        totalesY += 6;
      });
      ...
      doc.text(`$${cotizacion.total.toLocaleString('es-CO', { minimumFractionDigits: 2 })}`, 190, totalesY + 4, { align: 'right' });
    }
    ...
    if (cotizacion.observaciones) {
      ...
      const splitObservaciones = doc.splitTextToSize(cotizacion.observaciones, 170);
  ...
  const openEditModal = (cotizacion: Cotizacion) => {
    ...
    setProductosOrden(cotizacion.productos || []);
    setSelectedProductoId('');
    setCantidadProducto(1);
    setPrecioProducto(0);
  ...
  const openDeleteModal = (cotizacion: Cotizacion) => {
    setSelectedCotizacion(cotizacion);
    setIsDeleteModalOpen(true);
  };

  const openViewModal = (cotizacion: Cotizacion) => {
    setSelectedCotizacion(cotizacion);
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Gestiona las cotizaciones de productos</p>
      </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats.totalCotizaciones}</p>
            </div>
              ...
              {filteredCotizaciones.length === 0 ? (
                ...
                currentCotizaciones.map((cotizacion, index) => (
                  <TableRow key={cotizacion.id} className="hover:bg-gray-50">
                    ...
                    <TableCell className="font-medium">{cotizacion.numeroCotizacion}</TableCell>
                    <TableCell>{cotizacion.cliente}</TableCell>
                    <TableCell>{new Date(cotizacion.fecha).toLocaleDateString('es-CO')}</TableCell>
                    <TableCell>{new Date(cotizacion.fechaVencimiento).toLocaleDateString('es-CO')}</TableCell>
                    <TableCell>
                      <Badge variant="outline" className={getEstadoBadgeClass(cotizacion.estado)}>
                        {cotizacion.estado}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex items-center justify-center gap-2">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => openViewModal(cotizacion)}
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => openDeleteModal(cotizacion)}
        ...
        {filteredCotizaciones.length > 0 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200">
            <div className="text-sm text-gray-600">
              Mostrando {startIndex + 1} - {Math.min(endIndex, filteredCotizaciones.length)} de{' '}
      ...
      <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto" aria-describedby="create-quote-description">
          <DialogHeader>
            ...
            <div className="border-t pt-4">
              ...
              <div className="mt-4 bg-gray-50 p-4 rounded-lg space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">${calcularTotales.subtotal.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
                    ...
                    {Object.entries(calcularTotales.impuestosPorPorcentaje)
                      ...
                      .map(([porcentaje, monto]) => (
                        <div key={porcentaje} className="flex justify-between text-sm">
                          <span className="text-gray-600">Total IVA {porcentaje}%:</span>
                          <span className="font-medium">${monto.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                      ))}
                  </div>
                )}
                <div className="flex justify-between text-lg border-t pt-2">
                  ...
                  <span className="font-bold text-blue-600">${calcularTotales.total.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              </div>
            </div>
      ...
      <Dialog open={isCreateClienteModalOpen} onOpenChange={setIsCreateClienteModalOpen}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto" onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()} aria-describedby="create-client-description">
          <button
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto" aria-describedby="edit-quote-description">
          <DialogHeader>
      ...
      <Dialog open={isViewModalOpen} onOpenChange={setIsViewModalOpen}>
        <DialogContent className="max-w-7xl max-h-[90vh] overflow-y-auto" aria-describedby="view-quote-description">
          <DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                <div>
                  ...
                  <p className="font-medium text-blue-600">{selectedCotizacion.numeroCotizacion}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Estado</p>
                  <Badge variant="outline" className={getEstadoBadgeClass(selectedCotizacion.estado)}>
                    {selectedCotizacion.estado}
                  </Badge>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Cliente</p>
                  <p className="font-medium">{selectedCotizacion.cliente}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Bodega</p>
                  <p className="font-medium">{selectedCotizacion.bodega}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Fecha</p>
                  <p className="font-medium">{new Date(selectedCotizacion.fecha).toLocaleDateString('es-CO')}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Fecha de Vencimiento</p>
                  <p className="font-medium">{new Date(selectedCotizacion.fechaVencimiento).toLocaleDateString('es-CO')}</p>
                </div>
              </div>

              {/* Productos */}
              {selectedCotizacion.productos && selectedCotizacion.productos.length > 0 && (
                <div>
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        {selectedCotizacion.productos.map((item, index) => (
                          <TableRow key={index}>
                            <TableCell className="font-medium">{item.producto.nombre}</TableCell>
                            <TableCell className="text-center">{item.cantidad}</TableCell>
                            <TableCell className="text-right">${item.precio.toLocaleString()}</TableCell>
              ...
              <div className="flex justify-end">
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 min-w-[300px]">
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Subtotal:</span>
                      <span className="font-medium">${selectedCotizacion.subtotal.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    {(() => {
                      const impuestosPorPorcentaje = calcularImpuestosPorPorcentaje(selectedCotizacion);
                    ...
                    <div className="flex justify-between text-lg border-t border-blue-300 pt-2">
                      ...
                      <span className="font-bold text-blue-600">${selectedCotizacion.total.toLocaleString('es-CO', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {selectedCotizacion.observaciones && (
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <p className="text-gray-700">{selectedCotizacion.observaciones}</p>
                  </div>
                </div>
              )}
            </div>
      ...
      <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <DialogContent className="max-w-lg" aria-describedby="delete-quote-description">
          <DialogHeader>
          ...
          {selectedCotizacion && (
            <div className="py-4">
              <p className="text-sm text-gray-600">
                Se eliminará la cotización <span className="font-medium text-gray-900">{selectedCotizacion.numeroCotizacion}</span> del cliente{' '}
                <span className="font-medium text-gray-900">{selectedCotizacion.cliente}</span>.
              </p>
      ...
      <Dialog open={showSuccessModal} onOpenChange={setShowSuccessModal}>
        <DialogContent className="max-w-lg">
          ...
          <div className="py-4">
            <p className="text-gray-700">La cotización ha sido registrada correctamente en el sistema.</p>
          </div>
          <DialogFooter>
      ...
      <Dialog open={isPdfOptionsModalOpen} onOpenChange={setIsPdfOptionsModalOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="py-4 space-y-3">
            ...
            <Button
              onClick={() => {
                if (cotizacionParaPdf) handleDownloadPDF(cotizacionParaPdf, false);
                setIsPdfOptionsModalOpen(false);
              }}
              variant="outline"
